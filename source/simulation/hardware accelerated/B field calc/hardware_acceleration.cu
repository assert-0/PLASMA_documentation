...
void Calculate_h(param par)
{
    ...
    TYPE C = par.IncA * par.IncB * par.j * MI * par.IncFi;
    TYPE bh = par.b / 2;
	
	TYPE dR = 0;
	TYPE dr = -bh;
	TYPE dFi = 0;
	dR += ((global_index / (par.nFi + 1) / (par.nB + 1)) % (par.nA + 1)) * par.IncA;
	if(dR == 0 && global_index > (par.nFi + 1) * (par.nB + 1))
		return;
	dr += ((global_index / (par.nFi + 1)) % (par.nB + 1)) * par.IncB;
	dFi += (global_index % (par.nFi + 1)) * par.IncFi;
	...
	par.res[global_index] = Temp * par.FiWeights[ki] * (Dh * G) / pow((E - F * G), 1.5f);
}


__global__
void Calculate_z(param par)
{
	int index = threadIdx.x;
	int global_index = blockIdx.x * blockDim.x + index;
	...
	ki = int(round(dFi / par.IncFi));
	G = cos(dFi);
	par.res[global_index] = Temp * par.FiWeights[ki] * C * ((Cz - Dz * G) / (pow((E - F * G), 1.5f)));
}
...
void Calculate_single_point(param par, int blocks, TYPE *resh, TYPE *resz)
{
	if(par.r < 0.0)
	{
		cudaMemset(resh, 0, blocks * NTHREADS * sizeof(TYPE));
		cudaMemset(resz, 0, blocks * NTHREADS * sizeof(TYPE));
		return;
	}
	
	par.res = resz;
	Calculate_z<<<blocks, NTHREADS>>>(par);
	
	par.res = resh;
	Calculate_h<<<blocks, NTHREADS>>>(par);
}
	
int last_alloc_size = 0;
TYPE *inh = nullptr, *inz = nullptr, *result_transfer_buff = nullptr, *local_buff = nullptr;
TYPE DWeights[4][4];
TYPE FiWeights[nFi + 1];
TYPE EdgeList1[nA + 1];
TYPE EdgeList2[nB + 1];
...
void Resource_startup(param *par, int num_ops)
{
	...
	cudaMalloc(&inh, num_ops * sizeof(TYPE) * blocks * NTHREADS);
	cudaMalloc(&inz, num_ops * sizeof(TYPE) * blocks * NTHREADS);
...
void Calculate_hardware_accelerated(int num_ops, TYPE *theta, TYPE *point_distance, 
									...
									TYPE inc_fi, TYPE *bh, TYPE *bz, TYPE *b_abs)
{
	...
	for(int a = 0; a < num_ops; a++)
	{
		...
		Calculate_single_point(par, blocks, inh + a * blocks * NTHREADS, inz + a * blocks * NTHREADS);
	}
	cudaDeviceSynchronize();
	#if DEBUG_COPPER_EXTENSION 
	...
	if(bh != nullptr)
	{
			...
			QueryPerformanceCounter((LARGE_INTEGER*)&start);
		#endif
		Cut_multiple_buffers(num_ops * blocks * NTHREADS, result_transfer_buff, inh, blocks * NTHREADS);
		#if DEBUG_COPPER_EXTENSION 
			QueryPerformanceCounter((LARGE_INTEGER*)&stop);
		...
		for(int a = 0; a < num_ops; a++)
		{
			...
			{
				bh[a] += local_buff[a * int(blocks * NTHREADS / REDUCTIONFACTOR + 1) + b];
			}
		}
		#if DEBUG_COPPER_EXTENSION 
			QueryPerformanceCounter((LARGE_INTEGER*)&stop);
	...
	if(bz != nullptr)
	{
			...
			QueryPerformanceCounter((LARGE_INTEGER*)&start);
		#endif
		Cut_multiple_buffers(num_ops * blocks * NTHREADS, result_transfer_buff, inz, blocks * NTHREADS);  
		#if DEBUG_COPPER_EXTENSION 
			QueryPerformanceCounter((LARGE_INTEGER*)&stop);
		...
		for(int a = 0; a < num_ops; a++)
		{
			...
			for(int b = 0; b < int(blocks * NTHREADS / REDUCTIONFACTOR + 1); b++)
			{
				bz[a] += local_buff[a * int(blocks * NTHREADS / REDUCTIONFACTOR + 1) + b];
			}
		}
		#if DEBUG_COPPER_EXTENSION 
			QueryPerformanceCounter((LARGE_INTEGER*)&stop);
...
void Setup()
{
	Bh = new TYPE [NREPEATS];
	Bz = new TYPE [NREPEATS];
	B = new TYPE [NREPEATS];
	ZeroMemory(B, NREPEATS * sizeof(TYPE));
...
int main()
{
	...
	for(int a = 0; a < NREPEATS / CHUNKSIZE; a++)
	{
		...
		Calculate_hardware_accelerated(CHUNKSIZE, theta_ar + a * CHUNKSIZE, 
									   ...
									   inc_fi, Bh + a * CHUNKSIZE, Bz + a * CHUNKSIZE, 
									   B + a * CHUNKSIZE);
		#if DEBUG_COPPER_EXTENSION
	...
	printf("Done!\n");
	printf("%.15f %.15f %.15f\n", Bh[NREPEATS - 1], Bz[NREPEATS - 1], B[NREPEATS - 1]);
